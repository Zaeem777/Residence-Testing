...
def test_empty_password(page: Page) -> None:
    ...
    assert not is_valid  # should be False

    # Check the browser’s validation message
    message = password_input.evaluate("el => el.validationMessage")
    print("Validation message:", message)

    # Assert it contains the expected text (case-insensitive)
    assert "fill" in message.lower()

    # Also assert that URL did NOT change (still on login page)
